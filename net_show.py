...
def check(d):
    ...
    try:
        ...
        net_connect.enable()
        print("+"+"-"*78+"+")
        print("|"+"[ show commands ]".center(78)+"|")
        print("+"+"-"*78+"+")
        # Read show/exec commands from file and run them in EXEC mode
        with open("showrun.txt") as f:
            commands = [line.strip() for line in f if line.strip() and not line.strip().startswith("#")]
        for cmd in commands:
            output = net_connect.send_command(cmd, expect_string=r"#")
            print(f"--- {cmd} ---{output}")
        # No save_config here: we didn't change the running-config
